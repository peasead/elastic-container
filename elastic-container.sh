...
if fleet=0; then
  COMPOSE_STRING=" --file compose/no-fleet-docker-compose.yml"
else
  COMPOSE_STRING=" --file compose/docker-compose.yml"
fi
case "${ACTION}" in
...
"start")
  ...
  ${COMPOSE} up -d ${COMPOSE_STRING} --no-deps
